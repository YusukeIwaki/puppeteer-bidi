...
def with_cookie_context(browser)
...
test("[Cookie specs][Page.cookies] should return no cookies in pristine browser context") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    expect_cookie_equals(page.cookies, [], chrome: browser.user_agent.include?("Chrome"))
  end
end

test("[Cookie specs][Page.cookies] should get a cookie") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
...
test("[Cookie specs][Page.cookies] should properly report httpOnly cookie") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
...
test('[Cookie specs][Page.cookies] should properly report "Strict" sameSite cookie') do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    server.set_route('/empty.html') do |_request, writer|
...
test('[Cookie specs][Page.cookies] should properly report "Lax" sameSite cookie') do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    server.set_route('/empty.html') do |_request, writer|
...
test('[Cookie specs][Page.cookies] should properly report "Default" sameSite cookie') do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
...
test('[Cookie specs][Page.cookies] should be able to delete "Default" sameSite cookie') do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(name: 'a', value: 'b', sameSite: 'Default')
...
test('[Cookie specs][Page.cookies] should report "Default" sameSite cookie when not specified') do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    server.set_route('/empty.html') do |_request, writer|
...
test("[Cookie specs][Page.cookies] should get multiple cookies") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.evaluate(<<~JS)
...
test("[Cookie specs][Page.cookies] should get cookies from multiple urls") do |browser:|
  with_cookie_context(browser) do |page, _context|
    page.set_cookie(
      {
        url: 'https://foo.com',
...
test("[Cookie specs][Page.cookies] should get cookies from subdomain if the domain field allows it") do |browser:|
  with_cookie_context(browser) do |page, _context|
    page.set_cookie(
...
test("[Cookie specs][Page.cookies] should not get cookies from subdomain if the cookie is for top-level domain") do |browser:|
  with_cookie_context(browser) do |page, _context|
    page.set_cookie(
      url: 'https://base_domain.com',
      domain: 'base_domain.com',
...
test("[Cookie specs][Page.cookies] should get cookies from nested path") do |browser:|
  with_cookie_context(browser) do |page, _context|
    page.set_cookie(
      url: 'https://foo.com',
      path: '/some_path',
...
test("[Cookie specs][Page.cookies] should not get cookies from not nested path") do |browser:|
  with_cookie_context(browser) do |page, _context|
    page.set_cookie(
      url: 'https://foo.com',
      path: '/some_path',
...
test("[Cookie specs][Page.set_cookie] should work") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
...
test("[Cookie specs][Page.set_cookie] should isolate cookies in browser contexts") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.set_cookie] should set multiple cookies") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(
      {
...
test("[Cookie specs][Page.set_cookie] should have |expires| set to |-1| for session cookies") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(name: 'password', value: '123456')
    cookies = page.cookies
...
test("[Cookie specs][Page.set_cookie] should set cookie with reasonable defaults") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(name: 'password', value: '123456')
    cookies = page.cookies.sort_by { |cookie| cookie['name'] }
...
test("[Cookie specs][Page.set_cookie] should set cookie with all available properties") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(
      name: 'password',
...
test("[Cookie specs][Page.set_cookie] should set a cookie with a path") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.set_cookie] should set a cookie with a partitionKey") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
...
test("[Cookie specs][Page.set_cookie] should not set a cookie on a blank page") do |browser:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.set_cookie] should not set a cookie with blank page URL") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    error = nil
    page.goto(server.empty_page)
    begin
...
test("[Cookie specs][Page.set_cookie] should not set a cookie on a data URL page") do |browser:|
  with_cookie_context(browser) do |page, _context|
    error = nil
...
test("[Cookie specs][Page.set_cookie] should default to setting secure cookie for HTTPS websites") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
...
test("[Cookie specs][Page.set_cookie] should be able to set insecure cookie for HTTP website") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
...
test("[Cookie specs][Page.set_cookie] should set a cookie on a different domain") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(
...
test("[Cookie specs][Page.set_cookie] should set cookies from a frame") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto("#{server.prefix}/grid.html")
...
test("[Cookie specs][Page.set_cookie] should set secure same-site cookies from a frame") do |browser:, https_server:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.delete_cookie] should delete cookie") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    page.goto(server.empty_page)
    page.set_cookie(
      {
...
test("[Cookie specs][Page.delete_cookie] should not delete cookie for different domain") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.delete_cookie] should delete cookie for specified URL") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    cookie_destination_url = 'https://example.com'
    cookie_name = 'some_cookie_name'
...
test("[Cookie specs][Page.delete_cookie] should delete cookie for specified URL regardless of the current page") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.delete_cookie] should only delete cookie from the default partition if partitionkey is not specified") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
...
test("[Cookie specs][Page.delete_cookie] should delete cookie with partition key if partition key is specified") do |browser:, server:|
  with_cookie_context(browser) do |page, _context|
    origin = origin_for(server.empty_page)
    page.goto(server.empty_page)
    page.set_cookie(
