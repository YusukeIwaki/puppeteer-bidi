...
test("[QuerySelector][Page.query_selector] should query existing element") do |page:|
...
test("[QuerySelector][Page.query_selector] should return null for non-existing element") do |page:|
  page.set_content("<section>test</section>")
  element = page.query_selector("non-existing-element")
  expect(element).to be_nil
end

test("[QuerySelector][ElementHandle.query_selector] should query existing element") do |page:|
...
test("[QuerySelector][ElementHandle.query_selector] should return null for non-existing element") do |page:|
...
test("[QuerySelector][Page.query_selector_all] should query existing elements") do |page:|
...
test("[QuerySelector][Page.query_selector_all] should return empty array for non-existing elements") do |page:|
  page.set_content("<span>A</span>")
  elements = page.query_selector_all("div")
  expect(elements).to eq([])
end

test("[QuerySelector][ElementHandle.query_selector_all] should query existing elements") do |page:|
...
test("[QuerySelector][ElementHandle.query_selector_all] should return empty array for non-existing elements") do |page:|
...
test("[QuerySelector][Page.eval_on_selector] should work") do |page:|
...
test("[QuerySelector][Page.eval_on_selector] should accept arguments") do |page:|
  page.set_content("<section>hello</section>")
  text = page.eval_on_selector("section", "(e, suffix) => e.textContent + suffix", " world!")
  expect(text).to eq("hello world!")
end

test("[QuerySelector][Page.eval_on_selector] should accept ElementHandles as arguments") do |page:|
...
test("[QuerySelector][Page.eval_on_selector] should throw error if no element is found") do |page:|
  page.set_content("<section>test</section>")
  expect {
...
test("[QuerySelector][ElementHandle.eval_on_selector] should work") do |page:|
...
test("[QuerySelector][ElementHandle.eval_on_selector] should retrieve content from subtree") do |page:|
...
test("[QuerySelector][ElementHandle.eval_on_selector] should throw in case of missing selector") do |page:|
...
test("[QuerySelector][Page.eval_on_selector_all] should work") do |page:|
...
test("[QuerySelector][Page.eval_on_selector_all] should accept extra arguments") do |page:|
  page.set_content("<div>hello</div><div>beautiful</div><div>world!</div>")
  result = page.eval_on_selector_all("div", "(divs, two, three) => divs.length + two + three", 2, 3)
  expect(result).to eq(8)
end

test("[QuerySelector][Page.eval_on_selector_all] should accept ElementHandles as arguments") do |page:|
...
test("[QuerySelector][Page.eval_on_selector_all] should handle many elements") do |page:|
  page.evaluate(<<~JS)
    (() => {
...
test("[QuerySelector][ElementHandle.eval_on_selector_all] should retrieve content from subtree") do |page:|
...
test("[QuerySelector][ElementHandle.eval_on_selector_all] should not throw in case of missing selector") do |page:|
