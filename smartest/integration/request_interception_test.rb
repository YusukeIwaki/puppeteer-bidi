...
test("[Page.set_request_interception] should intercept") do |page:, server:|
...
test("[Page.set_request_interception] should work when POST is redirected with 302") do |page:, server:|
...
test("[Page.set_request_interception] should work with keep alive redirects") do |page:, server:|
...
test("[Page.set_request_interception] should work when header manipulation headers with redirect") do |page:, server:|
...
test("[Page.set_request_interception] should be able to remove headers") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
...
test("[Page.set_request_interception] should contain referer header") do |page:, server:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
...
test("[Page.set_request_interception] should not allow mutating request headers") do |page:, server:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
  requests << request unless is_favicon?(request)
...
test("[Page.set_request_interception] should work with requests without networkId") do |page:, server:|
...
test("[Page.set_request_interception] should properly return navigation response when URL has cookies") do |page:, server:|
page.goto(server.empty_page)
...
test("[Page.set_request_interception] should stop intercepting") do |page:, server:|
page.set_request_interception(true)
...
test("[Page.set_request_interception] should show custom HTTP headers") do |page:, server:|
...
test("[Page.set_request_interception] should work with redirect inside sync XHR") do |page:, server:|
page.goto(server.empty_page)
...
test("[Page.set_request_interception] should work with custom referer headers") do |page:, server:|
...
test("[Page.set_request_interception] should be abortable") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
...
test("[Page.set_request_interception] should be abortable with custom error codes") do |page:, server:|
...
test("[Page.set_request_interception] should send referer") do |page:, server:|
...
test("[Page.set_request_interception] should fail navigation when aborting main resource") do |page:, server:|
page.set_request_interception(true)
...
test("[Page.set_request_interception] should work with redirects") do |page:, server:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
  request.continue
...
test("[Page.set_request_interception] should work with redirects for subresources") do |page:, server:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
  request.continue
...
test("[Page.set_request_interception] should be able to abort redirects") do |page:, server:|
page.set_request_interception(true)
...
test("[Page.set_request_interception] should work with equal requests") do |page:, server:|
page.goto(server.empty_page)
response_count = 1
...
test("[Page.set_request_interception] should navigate to dataURL and fire dataURL requests") do |page:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
  requests << request unless is_favicon?(request)
...
test("[Page.set_request_interception] should be able to fetch dataURL and fire dataURL requests") do |page:, server:|
page.goto(server.empty_page)
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
...
test("[Page.set_request_interception] should navigate to URL with hash and fire requests without hash") do |page:, server:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
  request.continue
...
test("[Page.set_request_interception] should work with encoded server") do |page:, server:|
page.set_request_interception(true)
page.on(:request) { |request| request.continue }
...
test("[Page.set_request_interception] should work with badly encoded server") do |page:, server:|
page.set_request_interception(true)
...
test("[Page.set_request_interception] should work with missing stylesheets") do |page:, server:|
page.set_request_interception(true)
requests = []
page.on(:request) do |request|
  request.continue
...
test("[Page.set_request_interception] should not throw if the request was cancelled") do |page:, server:|
...
test("[Page.set_request_interception] should throw if interception is not enabled") do |page:, server:|
error = nil
page.on(:request) do |request|
  begin
    request.continue
...
test("[Page.set_request_interception] should work with file URLs") do |page:|
...
[
  { url: "/cached/one-style.html", resource_type: "stylesheet", cached_url: "/cached/one-style.css" },
  { url: "/cached/one-script.html", resource_type: "script", cached_url: "/cached/one-script.js" },
].each do |case_data|
  test("[Page.set_request_interception] should not cache #{case_data[:resource_type]} if cache disabled") do |page:, server:|
  ...
  test("[Page.set_request_interception] should cache #{case_data[:resource_type]} if cache enabled") do |page:, server:|
    page.goto("#{server.prefix}#{case_data[:url]}")
...
test("[Page.set_request_interception] should load fonts if cache enabled") do |page:, server:|
page.set_request_interception(true)
page.set_cache_enabled(true)
page.on(:request) { |request| request.continue }
...
test("[Page.set_request_interception] should work with worker") do |page:, server:|
...
test("[Request.continue] should work") do |page:, server:|
page.set_request_interception(true)
page.on(:request) { |request| request.continue }
page.goto(server.empty_page)
end

test("[Request.continue] should amend HTTP headers") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
  headers = request.headers.merge("foo" => "bar")
  request.continue(headers: headers)
...
test("[Request.continue] should redirect in a way non-observable to page") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
...
test("[Request.continue] should amend method") do |page:, server:|
page.goto(server.empty_page)
page.set_request_interception(true)
...
test("[Request.continue] should amend post data") do |page:, server:|
page.goto(server.empty_page)
page.set_request_interception(true)
...
test("[Request.continue] should amend both post data and method on navigation") do |page:, server:|
page.set_request_interception(true)
...
test("[Request.continue] should fail if the header value is invalid") do |page:, server:|
error = nil
page.set_request_interception(true)
page.on(:request) do |request|
  begin
...
test("[Request.respond] should work") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
  request.respond(
    status: 201,
...
test("[Request.respond] should work with status code 422") do |page:, server:|
page.set_request_interception(true)
...
test("[Request.respond] should redirect") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
...
test("[Request.respond] should allow mocking multiple headers with same key") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
  request.respond(
    status: 200,
...
test("[Request.respond] should allow mocking binary responses") do |page:, server:|
...
test("[Request.respond] should stringify intercepted request response headers") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
...
test("[Request.respond] should fail if the header value is invalid") do |page:, server:|
error = nil
page.set_request_interception(true)
page.on(:request) do |request|
  begin
...
test("[Request.respond] should report correct content-length header with string") do |page:, server:|
page.set_request_interception(true)
...
test("[Request.respond] should report correct content-length header with buffer") do |page:, server:|
page.set_request_interception(true)
...
test("[Request.respond] should report correct encoding from page when content-type is set") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
  request.respond(
    status: 200,
...
test("[Request.resource_type] should work for document type") do |page:, server:|
page.set_request_interception(true)
page.on(:request) { |request| request.continue }
response = page.goto(server.empty_page)
...
test("[Request.resource_type] should work for stylesheets") do |page:, server:|
page.set_request_interception(true)
css_requests = []
page.on(:request) do |request|
