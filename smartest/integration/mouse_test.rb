...
test("[Mouse] should click the document") do |page:|
  page.evaluate(<<~JS)
    () => {
...
test("[Mouse] should resize the textarea") do |page:, server:|
  page.goto("#{server.prefix}/input/textarea.html")
  dim = mouse_dimensions(page)
  x = dim["x"]
  y = dim["y"]
  width = dim["width"]
  height = dim["height"]
  ...
  new_dim = mouse_dimensions(page)
  expect(new_dim["width"]).to eq((width + 104).round)
  expect(new_dim["height"]).to eq((height + 104).round)
end

test("[Mouse] should select the text with mouse") do |page:, server:|
...
test("[Mouse] should trigger hover state") do |page:, server:|
...
test("[Mouse] should trigger hover state with removed window.Node") do |page:, server:|
  page.goto("#{server.prefix}/input/scrollable.html")
...
test("[Mouse] should set modifier keys on click") do |page:, server:|
  page.goto("#{server.prefix}/input/scrollable.html")
  page.evaluate(<<~JS)
    () => {
...
test("[Mouse] should send mouse wheel events") do |page:, server:|
...
test("[Mouse] should set ctrlKey on the wheel event") do |page:, server:|
...
test("[Mouse] should tween mouse movement") do |page:|
  page.mouse.move(100, 100)
  page.evaluate(<<~JS)
    () => {
      globalThis.result = [];
...
test("[Mouse] should work with mobile viewports and cross process navigations") do |page:, server:|
...
test("[Mouse] should not throw if buttons are pressed twice") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Mouse] should not throw if clicking in parallel") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Mouse] should reset properly") do |page:, server:|
  page.goto(server.empty_page)
  ...
  expect(data.length).to be_greater_than_or_equal_to(2)
...
test("[Mouse] should evaluate before mouse event") do |page:, server:|
  page.goto(server.empty_page)
