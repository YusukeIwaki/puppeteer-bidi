...
# Helpers shared by Frame.waitForSelector tests

ADD_ELEMENT_JS = <<~JAVASCRIPT
  (tag) => {
    return document.body.appendChild(document.createElement(tag));
  }
JAVASCRIPT

def waittask_attach_frame(page, frame_id, url)
...
def waittask_detach_frame(page, frame_id)
...
test("[Frame.waitForFunction] should accept a string") do |page:, server:|
...
test("[Frame.waitForFunction] should accept a string with block") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should work when resolved right before execution context disposal") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should poll on interval") do |page:, server:|
  page.goto(server.empty_page)
  ...
  expect(elapsed).to be_greater_than_or_equal_to(250)
end

test("[Frame.waitForFunction] should poll on interval with block") do |page:, server:|
  page.goto(server.empty_page)
  page.evaluate("() => { delete window.__FOO }")
...
test("[Frame.waitForFunction] should poll on mutation") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should poll on mutation async") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should poll on raf") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should poll on raf async") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should work with strict CSP policy") do |page:, server:|
...
test("[Frame.waitForFunction] should throw on negative polling interval") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should return the success value as a JSHandle") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should return the window as a success value") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should accept ElementHandle arguments") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should respect timeout") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should respect default timeout") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should disable timeout when set to 0") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForFunction] should survive cross-process navigation") do |page:, server:|
  foo_found = false
...
test("[Frame.waitForFunction] should survive navigations") do |page:, server:|
  done = false
...
test("[Frame.waitForFunction] should be cancellable with an abort signal") do
  skip "Requires JS-specific AbortController/signal support."
end

test("[Frame.waitForFunction] should not cause an unhandled error when aborted") do
  skip "Requires JS-specific AbortController/signal support."
end

test("[Frame.waitForFunction] can start multiple tasks without warnings when aborted") do
  skip "Requires JS-specific AbortController/signal support."
end

# ===== Frame.waitForSelector =====

test("[Frame.waitForSelector] should immediately resolve promise if node exists") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector] should be cancellable") do
  skip "AbortController/signal support needed for cancellation, which is JS-specific."
end

test("[Frame.waitForSelector] should work with removed MutationObserver") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector] should resolve promise when node is added") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector] should work when node is added through innerHTML") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector] should work when node is added in a shadow root") do |page:, server:|
  pending "Shadow root piercing selector not supported"
...
test("[Frame.waitForSelector] should work for selector with a pseudo class") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector] Page.waitForSelector is shortcut for main frame") do |page:, server:|
  page.goto(server.empty_page)

  other_frame = waittask_attach_frame(page, "frame1", server.empty_page)
...
test("[Frame.waitForSelector] should run in specified frame") do |page:, server:|
  page.goto(server.empty_page)

  frame1 = waittask_attach_frame(page, "frame1", server.empty_page)
  frame2 = waittask_attach_frame(page, "frame2", server.empty_page)
...
test("[Frame.waitForSelector] should throw when frame is detached") do |page:, server:|
  page.goto(server.empty_page)

  frame = waittask_attach_frame(page, "frame1", server.empty_page)

  wait_error = nil

  begin
    frame.wait_for_selector(".box") do
      waittask_detach_frame(page, "frame1")
    end
  rescue => error
    wait_error = error
  end
...
test("[Frame.waitForSelector] should survive cross-process navigation") do |page:, server:|
  navigation_task = nil
...
test("[Frame.waitForSelector] should wait for element to be visible (display)") do |page:|
...
test("[Frame.waitForSelector] should wait for element to be visible without DOM mutations") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be visible (visibility)") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be visible (bounding box)") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be visible recursively") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be hidden (visibility)") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be hidden (display)") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be hidden (bounding box)") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should wait for element to be hidden (removal)") do |page:|
  promise_resolved = false
...
test("[Frame.waitForSelector] should return nil if waiting to hide non-existing element") do |page:|
  handle = page.wait_for_selector("non-existing", hidden: true)
  expect(handle).to be_nil
end

test("[Frame.waitForSelector] should respect timeout") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector] should have an error message specifically for awaiting hidden element") do |page:|
  page.set_content("<div>text</div>")
...
test("[Frame.waitForSelector] should respond to node attribute mutation") do |page:|
  handle = page.wait_for_selector(".zombo") do
...
test("[Frame.waitForSelector] should return the element handle") do |page:|
  handle = page.wait_for_selector(".zombo") do
    page.set_content('<div class="zombo">anything</div>')
  end
...
test("[Frame.waitForSelector] should have correct stack trace for timeout") do |page:|
  expect {
...
# ===== Frame.waitForSelector > xpath =====

test("[Frame.waitForSelector][xpath] should support some fancy xpath") do |page:|
...
test("[Frame.waitForSelector][xpath] should respect timeout") do |page:|
  expect {
    page.wait_for_selector("xpath/.//div", timeout: 10)
  }.to raise_error(Puppeteer::Bidi::TimeoutError, /Waiting for selector `\.\/\/div` failed/)
end

test("[Frame.waitForSelector][xpath] should run in specified frame") do |page:, server:|
  page.goto(server.empty_page)

  waittask_attach_frame(page, "frame1", server.empty_page)
  frame2 = waittask_attach_frame(page, "frame2", server.empty_page)
...
test("[Frame.waitForSelector][xpath] should throw when frame is detached") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame.waitForSelector][xpath] hidden should wait for display: none") do |page:|
  page.set_content('<div style="display: block;">text</div>')
...
test("[Frame.waitForSelector][xpath] hidden should return nil if the element is not found") do |page:|
  handle = page.wait_for_selector("xpath/.//div", hidden: true)
  expect(handle).to be_nil
end

test("[Frame.waitForSelector][xpath] hidden should return an empty element handle if the element is found") do |page:|
  page.set_content('<div style="display: none;">text</div>')
...
test("[Frame.waitForSelector][xpath] should return the element handle") do |page:|
...
test("[Frame.waitForSelector][xpath] should allow you to select a text node") do |page:|
...
test("[Frame.waitForSelector][xpath] should allow you to select an element with single slash") do |page:|
  page.set_content("<div>some text</div>")
