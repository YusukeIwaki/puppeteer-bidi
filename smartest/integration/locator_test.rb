...
test("[Locator] should work with a frame") do |page:|
  button = nil
  begin
...
test("[Locator] should work without preconditions") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work for multiple selectors") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work if the element is out of viewport") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work with element handles") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work if the element becomes visible later") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work if the element becomes enabled later") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.click] should work if multiple conditions are satisfied later") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
...
test("[Locator][Locator.click] should time out") do |page:|
...
test("[Locator][Locator.click] should retry clicks on errors") do |page:|
  page.set_default_timeout(200)
  page.set_viewport(width: 500, height: 500)
  page.set_content(<<~HTML)
    <button style="display: none;" onclick="this.innerText = 'clicked';">
...
test("[Locator][Locator.click] can be aborted") do
  skip "Abort signals are not implemented for locator actions in Ruby yet."
end

test("[Locator][Locator.click] should work with an iframe") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.hover] should work") do |page:|
  button = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.scroll] should work") do |page:|
  scrollable = nil
  begin
    page.set_viewport(width: 500, height: 500)
    page.set_content(<<~HTML)
...
test("[Locator][Locator.fill] should work for textarea") do |page:|
...
test("[Locator][Locator.fill] should work for selects") do |page:|
  page.set_content(<<~HTML)
    <select>
...
test("[Locator][Locator.fill] should work for inputs") do |page:|
...
test("[Locator][Locator.fill] should work for large text") do |page:|
  page.set_content("<textarea></textarea>")
  text = "a" * 1000
...
test("[Locator][Locator.fill] should work for large text in contenteditable") do |page:|
...
test("[Locator][Locator.fill] should work with a custom typing threshold") do |page:|
  page.set_content("<input />")
...
test("[Locator][Locator.fill] should work if the input becomes enabled later") do |page:|
  input = nil
  begin
...
test("[Locator][Locator.fill] should work for contenteditable") do |page:|
  page.set_content('<div contenteditable="true"></div>')
  page.locator("div").fill("test")
  expect(page.evaluate("() => document.querySelector('div')?.innerText === 'test'")).to eq(true)
end

test("[Locator][Locator.fill] should work for pre-filled inputs") do |page:|
  page.set_content('<input value="te" />')
  page.locator("input").fill("test")
  expect(page.evaluate("() => document.querySelector('input')?.value === 'test'")).to eq(true)
end

test("[Locator][Locator.fill] should override pre-filled inputs") do |page:|
  page.set_content('<input value="wrong prefix" />')
  page.locator("input").fill("test")
  expect(page.evaluate("() => document.querySelector('input')?.value === 'test'")).to eq(true)
end

test("[Locator][Locator.fill] should work for non-text inputs") do |page:|
...
test("[Locator][Locator.race] races multiple locators") do |page:|
  page.set_viewport(width: 500, height: 500)
  page.set_content(<<~HTML)
...
test("[Locator][Locator.race] can be aborted") do
  skip "Abort signals are not implemented for locator actions in Ruby yet."
end

test("[Locator][Locator.race] should time out when all locators do not match") do |page:|
...
test("[Locator][Locator.race] should not time out when one of the locators matches") do |page:|
  page.set_content("<button>test</button>")
  Puppeteer::Bidi::Locator.race([
    page.locator("not-found"),
    page.locator("button"),
...
test("[Locator][Locator.prototype.map] should work") do |page:|
...
test("[Locator][Locator.prototype.map] should work with throws") do |page:|
  page.set_content("<div>test</div>")
...
test("[Locator][Locator.prototype.map] should work with expect") do |page:|
  page.set_content("<div>test</div>")
  result = Thread.new do
    page
      .locator("::-p-text(test)")
...
test("[Locator][Locator.prototype.filter] should resolve as soon as the predicate matches") do |page:|
  page.set_content("<div>test</div>")
  result = Thread.new do
    page
      .locator("::-p-text(test)")
...
test("[Locator][Locator.prototype.wait] should work") do |page:|
  page.set_content(<<~HTML)
    <script>
      setTimeout(() => {
...
test("[Locator][Locator.prototype.wait_handle] should work") do |page:|
  handle = nil
  begin
    page.set_content(<<~HTML)
      <script>
...
test("[Locator][Locator.prototype.clone] should work") do |page:|
  locator = page.locator("div")
  clone = locator.clone
  expect(locator).not_to eq(clone)
end

test("[Locator][Locator.prototype.clone] should work internally with delegated locators") do |page:|
  locator = page.locator("div")
  delegated_locators = [
...
test("[Locator][FunctionLocator] should work") do |page:|
  result = Thread.new do
    page
      .locator(function: <<~JS)
        () => {
...
test("[Locator][FunctionLocator] should work with actions") do |page:|
