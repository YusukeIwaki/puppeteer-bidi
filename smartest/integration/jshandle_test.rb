...
test(['JSHandle', 'Page.evaluateHandle', 'should work'].join(" ")) do |page:, server:|
  window_handle = page.evaluate_handle('window')
  expect(window_handle).not_to be_nil
  expect(window_handle).to be_a(Puppeteer::Bidi::JSHandle)
end

test(['JSHandle', 'Page.evaluateHandle', 'should return the RemoteObject'].join(" ")) do |page:, server:|
  window_handle = page.evaluate_handle('window')
...
test(['JSHandle', 'Page.evaluateHandle', 'should accept object handle as an argument'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#json_value', 'should work'].join(" ")) do |page:, server:|
  a_handle = page.evaluate_handle("({ foo: 'bar' })")
  json = a_handle.json_value
  expect(json).to eq({ 'foo' => 'bar' })
end

test(['JSHandle', 'JSHandle#json_value', 'should work with jsonValues that are not objects'].join(" ")) do |page:, server:|
  a_handle = page.evaluate_handle('[1, 2, 3]')
  json = a_handle.json_value
  expect(json).to eq([1, 2, 3])
end

test(['JSHandle', 'JSHandle#json_value', 'should work with jsonValues that are primitives'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#json_value', 'should work with dates'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#json_value', 'should not throw for circular objects'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#get_properties', 'should return even non-own properties'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#as_element', 'should work'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#as_element', 'should return null for non-elements'].join(" ")) do |page:, server:|
  a_handle = page.evaluate_handle('2')
  element = a_handle.as_element
  expect(element).to be_nil
end

test(['JSHandle', 'JSHandle#as_element', 'should return ElementHandle for TextNodes'].join(" ")) do |page:, server:|
...
test(['JSHandle', 'JSHandle#to_s', 'should work for complicated objects'].join(" ")) do |page:, server:|
  a_handle = page.evaluate_handle('window')
  expect(a_handle.to_s).to eq('JSHandle@window')
end

test(['JSHandle', 'JSHandle#to_s', 'should work with different subtypes'].join(" ")) do |page:, server:|
  test_cases = {
    'function' => '(function(){})',
    'array' => '[1, 2, 3]',
    'regexp' => '/foo/',
