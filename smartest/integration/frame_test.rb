...
test("[Frame specs][Frame.evaluateHandle] should work") do |page:, server:|
...
test("[Frame specs][Frame.evaluate] should throw for detached frames") do |page:, server:|
...
test("[Frame specs][Frame.evaluate] allows readonly array to be an argument") do |page:, server:|
  page.goto(server.empty_page)
  main_frame = page.main_frame
...
test("[Frame specs][Frame.page] should retrieve the page from a frame") do |page:, server:|
  page.goto(server.empty_page)
  main_frame = page.main_frame
  expect(main_frame.page).to eq(page)
end

test("[Frame specs][Frame Management] should handle nested frames") do |page:, server:|
...
test("[Frame specs][Frame Management] should send events when frames are manipulated dynamically") do |page:, server:|
  page.goto(server.empty_page)
...
test('[Frame specs][Frame Management] should send "framenavigated" when navigating on anchor URLs') do |page:, server:|
  page.goto(server.empty_page)
...
test("[Frame specs][Frame Management] should persist mainFrame on cross-process navigation") do |page:, server:|
...
test("[Frame specs][Frame Management] should not send attach/detach events for main frame") do |page:, server:|
  has_events = false
...
test("[Frame specs][Frame Management] should detach child frames on navigation") do |page:, server:|
  attached_frames = []
  detached_frames = []
  navigated_frames = []
  page.on(:frameattached) { |frame| attached_frames << frame }
...
test("[Frame specs][Frame Management] should support framesets") do |page:, server:|
...
test("[Frame specs][Frame Management] should click elements in a frameset") do |page:, server:|
...
test("[Frame specs][Frame Management] should report frame from inside shadow DOM") do |page:, server:|
...
test("[Frame specs][Frame Management] should report frame.parent()") do |page:, server:|
...
test("[Frame specs][Frame Management] should report different frame instance when frame re-attaches") do |page:, server:|
...
test("[Frame specs][Frame Management] should support url fragment") do |page:, server:|
...
test("[Frame specs][Frame Management] should support lazy frames") do |page:, server:|
...
test("[Frame specs][Frame.client] should return the client instance") do
  skip "Frame.client is CDP-specific, not applicable to WebDriver BiDi"
end

test("[Frame specs][Frame.frameElement] should work") do |page:, server:|
...
test("[Frame specs][Frame.frameElement] should handle shadow roots") do |page:|
...
test("[Frame specs][Frame.frameElement] should return ElementHandle in the correct world") do |page:, server:|
