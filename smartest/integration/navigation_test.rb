...
test("[Page.waitForNavigation] should work") do |page:, server:|
...
test("[Page.waitForNavigation] should navigate successfully after encountering network error") do |page:, server:|
...
test("[Page.waitForNavigation] should work with both domcontentloaded and load") do |page:, server:|
...
test("[Page.waitForNavigation] should work with clicking on anchor links") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Page.waitForNavigation] should work with history.pushState()") do |page:, server:|
  page.goto(server.empty_page)
...
test("[Page.waitForNavigation] should work with history.replaceState()") do |page:, server:|
  page.goto(server.empty_page)
  page.set_content(<<~HTML)
...
test("[Page.waitForNavigation] should work with DOM history.back()/history.forward()") do |page:, server:|
  page.goto(server.empty_page)
  page.set_content(<<~HTML)
...
test("[Page.waitForNavigation] should work when subframe issues window.stop()") do
  skip "Requires frame attachment event handling and complex navigation scenarios"
end

test("[Page.waitForNavigation] should be cancellable") do
