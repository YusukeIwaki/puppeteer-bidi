...
test(['Page.waitForNavigation', 'should work'].join(" ")) do |page:, server:|
...
test(['Page.waitForNavigation', 'should navigate successfully after encountering network error'].join(" ")) do |page:, server:|
...
test(['Page.waitForNavigation', 'should work with both domcontentloaded and load'].join(" ")) do |page:, server:|
...
test(['Page.waitForNavigation', 'should work with clicking on anchor links'].join(" ")) do |page:, server:|
  page.goto(server.empty_page)
...
test(['Page.waitForNavigation', 'should work with history.pushState()'].join(" ")) do |page:, server:|
  page.goto(server.empty_page)
...
test(['Page.waitForNavigation', 'should work with history.replaceState()'].join(" ")) do |page:, server:|
  page.goto(server.empty_page)
  page.set_content(<<~HTML)
...
test(['Page.waitForNavigation', 'should work with DOM history.back()/history.forward()'].join(" ")) do |page:, server:|
  page.goto(server.empty_page)
  page.set_content(<<~HTML)
...
test(['Page.waitForNavigation', 'should work when subframe issues window.stop()'].join(" ")) do
  skip 'Requires frame attachment event handling and complex navigation scenarios'
end

test(['Page.waitForNavigation', 'should be cancellable'].join(" ")) do
