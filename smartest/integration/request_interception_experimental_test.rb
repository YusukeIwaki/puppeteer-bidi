...
%w[abort continue respond].each do |expected_action|
  test("[Page.set_request_interception] should cooperatively #{expected_action} by priority") do |page:, server:|
    action_results = []
...
test("[Page.set_request_interception] should be able to access the error reason") do |page:, server:|
page.set_request_interception(true)
...
[
  ...
  test("[Page.set_request_interception] should cache #{case_data[:resource_type]} if cache enabled") do |page:, server:|
    ...
    page.reload
    expect(cached.length).to be_greater_than_or_equal_to(1)
  end
end
...
test("[Request.respond] should be able to access the response") do |page:, server:|
page.set_request_interception(true)
page.on(:request) do |request|
...
test("[Request.respond] should indicate already-handled if an intercept has been handled") do |page:, server:|
page.set_request_interception(true)
