...
require "set"
  ...
  def wait_for_event(emitter, event, timeout: 5)
    ...
    %w[abort continue respond].each do |expected_action|
      test(["Cooperative request interception", "Page.set_request_interception", "should cooperatively #{expected_action} by priority"].join(" ")) do |page:, server:|
        action_results = []
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should intercept"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      request_error = nil
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work when header manipulation headers with redirect"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should be able to remove headers"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should contain referer header"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      requests = []
      page.on(:request) do |request|
        request.continue({}, 0)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should properly return navigation response when URL has cookies"].join(" ")) do |page:, server:|
      page.goto(server.empty_page)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should stop intercepting"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should show custom HTTP headers"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with custom referer headers"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should be able to access the error reason"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should be abortable with custom error codes"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should send referer"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should fail navigation when aborting main resource"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) { |request| request.abort("failed", 0) }
      error = nil
      begin
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with redirects for subresources"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      requests = []
      page.on(:request) do |request|
        request.continue({}, 0)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should be able to abort redirects"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with equal requests"].join(" ")) do |page:, server:|
      page.goto(server.empty_page)
      response_count = 1
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should navigate to dataURL and fire dataURL requests"].join(" ")) do |page:|
      page.set_request_interception(true)
      requests = []
      page.on(:request) do |request|
        requests << request unless is_favicon?(request)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should be able to fetch dataURL and fire dataURL requests"].join(" ")) do |page:, server:|
      page.goto(server.empty_page)
      page.set_request_interception(true)
      requests = []
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should navigate to URL with hash and fire requests without hash"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      requests = []
      page.on(:request) do |request|
        requests << request unless is_favicon?(request)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with encoded server"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) { |request| request.continue({}, 0) }
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with badly encoded server"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with missing stylesheets"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      requests = []
      page.on(:request) do |request|
        request.continue({}, 0)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should not throw if the request was cancelled"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should throw if interception is not enabled"].join(" ")) do |page:, server:|
      error = nil
      page.on(:request) do |request|
        begin
          request.continue({}, 0)
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should work with file URLs"].join(" ")) do |page:|
    ...
    [
      ...
      test(["Cooperative request interception", "Page.set_request_interception", "should not cache #{case_data[:resource_type]} if cache disabled"].join(" ")) do |page:, server:|
      ...
      test(["Cooperative request interception", "Page.set_request_interception", "should cache #{case_data[:resource_type]} if cache enabled"].join(" ")) do |page:, server:|
        page.goto("#{server.prefix}#{case_data[:url]}")
    ...
    test(["Cooperative request interception", "Page.set_request_interception", "should load fonts if cache enabled"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.set_cache_enabled(true)
      page.on(:request) { |request| request.continue({}, 0) }
    ...
    test(["Cooperative request interception", "Request.continue", "should work"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) { |request| request.continue({}, 0) }
      page.goto(server.empty_page)
    end

    test(["Cooperative request interception", "Request.continue", "should amend HTTP headers"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
        headers = request.headers.merge("foo" => "bar")
        request.continue({ headers: headers }, 0)
    ...
    test(["Cooperative request interception", "Request.continue", "should redirect in a way non-observable to page"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Request.continue", "should amend method"].join(" ")) do |page:, server:|
      page.goto(server.empty_page)
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Request.continue", "should amend post data"].join(" ")) do |page:, server:|
      page.goto(server.empty_page)
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Request.continue", "should amend both post data and method on navigation"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Request.respond", "should work"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
        request.respond(
          {
    ...
    test(["Cooperative request interception", "Request.respond", "should be able to access the response"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Request.respond", "should work with status code 422"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Request.respond", "should redirect"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) do |request|
    ...
    test(["Cooperative request interception", "Request.respond", "should allow mocking binary responses"].join(" ")) do |page:, server:|
    ...
    test(["Cooperative request interception", "Request.respond", "should indicate already-handled if an intercept has been handled"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
    ...
    test(["Cooperative request interception", "Request.resource_type", "should work for document type"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      page.on(:request) { |request| request.continue({}, 0) }
      response = page.goto(server.empty_page)
    ...
    test(["Cooperative request interception", "Request.resource_type", "should work for stylesheets"].join(" ")) do |page:, server:|
      page.set_request_interception(true)
      css_requests = []
      page.on(:request) do |request|
